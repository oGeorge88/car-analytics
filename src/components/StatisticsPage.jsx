...
const fetchUrl = `${import.meta.env.VITE_BASE_PATH}/cars.json`;
...
const StatisticsPage = () => {
  ...
  const modelLabels = [
    ...new Set(
      Object.keys(data.models).reduce((acc, brand) => {
  ...
  const stackedBarData = {
    ...
    datasets: modelLabels.map((model, index) => {
      const dataForModel = Object.keys(data.models).map(
        (brand) => data.models[brand][model] || 0
      );
      return {
        label: model,
        data: dataForModel,
            ...
            <tbody>
              {Object.entries(data.brands).map(([brand, { count, value }]) => (
                <>
                  <tr key={brand}>
                    <td
                      rowSpan={Object.keys(data.models[brand] || {}).length + 1}
                    >
                      {brand}
                    </td>
                    <td>Total</td>
                  ...
                  {Object.entries(data.models[brand] || {}).map(
                    ([model, modelCount]) => (
                      <tr key={`${brand}-${model}`}>
                        <td>{model}</td>
                        <td>{modelCount}</td>
